 ...
 * @author Kai Burjack
 */
public class KhrRayTracingTriangleDemo {
    ...
    private static class WindowAndCallbacks {
        ...
        private int width;
        private int height;
        private WindowAndCallbacks(long window, int width, int height, GLFWKeyCallback keyCallback) {
            this.window = window;
            this.width = width;
            this.height = height;
    ...
    private static VkDevice createDevice(List<String> requiredExtensions) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            PointerBuffer extensions = stack.mallocPointer(1 + requiredExtensions.size());
    ...
    private static ColorFormatAndSpace determineSurfaceFormat(VkPhysicalDevice physicalDevice, long surface) {
        try (MemoryStack stack = stackPush()) {
            ...
            VkSurfaceFormatKHR.Buffer pSurfaceFormats = VkSurfaceFormatKHR
                    .callocStack(pSurfaceFormatCount.get(0), stack);
    ...
    private static class AllocationAndBuffer {
        ...
        private long mapped;
        private AllocationAndBuffer(long allocation, long buffer) {
                        ...
                        .pGeometries(VkAccelerationStructureGeometryKHR
                                ...
                                .geometry(VkAccelerationStructureGeometryDataKHR
                                        ...
                                        .instances(VkAccelerationStructureGeometryInstancesDataKHR
                                                ...
                                                .data(deviceAddressConst(stack, instanceData.buffer, 16))))
                                .flags(VK_GEOMETRY_OPAQUE_BIT_KHR))
                        .geometryCount(1);
    ...
    private static RayTracingPipeline createRayTracingPipeline() throws IOException {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            // load shaders
            String pkg = KhrRayTracingTriangleDemo.class.getPackage().getName().replace('.', '/') + "/";
            loadShader(pStages
                    .get(0)
                    .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO), 
                    null, stack, device, pkg + "khrraytracing/raygen.glsl", VK_SHADER_STAGE_RAYGEN_BIT_KHR);
            loadShader(pStages
                    .get(1)
                    .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO),
                    null, stack, device, pkg + "khrraytracing/raymiss.glsl", VK_SHADER_STAGE_MISS_BIT_KHR);
            loadShader(pStages
                    .get(2)
                    .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO),
                    null, stack, device, pkg + "khrraytracing/closesthit.glsl", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        init();
        Thread updateAndRenderThread = new Thread(KhrRayTracingTriangleDemo::runOnRenderThread);
